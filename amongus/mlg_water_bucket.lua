...
minetest.register_craftitem(modname .. ":totem", {
	description = S("OMG MLG"),
	...
	_mcl_wieldview_item = modname .. ":totem_wielded",
})
minetest.register_alias("mobs_mc:totem", modname .. ":totem")

minetest.register_craftitem(modname .. ":totem_wielded", {
	inventory_image = "bucket_water.png",
	wield_image = "bucket_water.png",
...
mcl_damage.register_modifier(function(obj, damage, reason)
	if obj:is_player() and not reason.flags.bypasses_totem then
		...
		if hp - damage <= 0 then
			...
			if wield:get_name() ~= modname .. ":totem" then
