...
local function orientate_dispenser(pos, placer)
	...
	if pitch > 55 then
		minetest.swap_node(pos, { name = modname .. ":creative_dispenser_up", param2 = node.param2 })
	elseif pitch < -55 then
		minetest.swap_node(pos, { name = modname .. ":creative_dispenser_down", param2 = node.param2 })
	end
end
	...
	mesecons = {
		effector = {
			...
			action_on = function(pos, node)
				...
				if node.name == modname .. ":creative_dispenser" then
					dropdir = vector.multiply(minetest.facedir_to_dir(node.param2), -1)
					droppos = vector.add(pos, dropdir)
				elseif node.name == modname .. ":creative_dispenser_up" then
					dropdir = vector.new(0, 1, 0)
					droppos = vector.offset(pos, 0, 1, 0)
				elseif node.name == modname .. ":creative_dispenser_down" then
					dropdir = vector.new(0, -1, 0)
					droppos = vector.offset(pos, 0, -1, 0)
				end
...
horizontal_def.description = S("Creative Dispenser")
...
minetest.register_node(modname .. ":creative_dispenser", horizontal_def)

-- Down dispenser
local down_def = table.copy(dispenserdef)
down_def.description = S("Downwards-Facing Creative Dispenser")
...
down_def.drop = modname .. ":creative_dispenser"
minetest.register_node(modname .. ":creative_dispenser_down", down_def)

-- Up dispenser
-- The up dispenser is almost identical to the down dispenser , it only differs in textures
local up_def = table.copy(down_def)
up_def.description = S("Upwards-Facing Creative Dispenser")
up_def.tiles = {
	"mcl_dispensers_dispenser_front_vertical.png", "default_furnace_bottom.png",
	"default_furnace_side.png", "default_furnace_side.png",
	"default_furnace_side.png", "default_furnace_side.png"
}
minetest.register_node(modname .. ":creative_dispenser_up", up_def)


minetest.register_craft({
	output = modname .. ":creative_dispenser",
	recipe = {
...
if minetest.get_modpath("doc") then
	doc.add_entry_alias("nodes", modname .. ":creative_dispenser", "nodes", modname .. ":creative_dispenser_down")
	doc.add_entry_alias("nodes", modname .. ":creative_dispenser", "nodes", modname .. ":creative_dispenser_up")
end

-- Legacy
-- minetest.register_lbm({
-- 	label = "Update dispenser formspecs (0.60.0)",
-- 	name = modname .. ":update_formspecs_0_60_0",
-- 	nodenames = { modname .. ":creative_dispenser", modname .. ":creative_dispenser_down", modname .. ":creative_dispenser_up" },
